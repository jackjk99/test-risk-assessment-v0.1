...
def analyze_image(image: Image, checklist: pd.DataFrame) -> pd.DataFrame:
    ...
    Example:
        Input Image: 현장 이미지 (분전반 작업 사진 등)
        Reference Checklist: SGR현장 체크리스트_변환.xlsx
        
        Output:
        | 번호  | 잠재 위험요인       | 잠재 위험요인 설명               | 위험성 감소대책 |
        |-------|--------------------|--------------------------------|----------------|
        | 1     | 고소 작업 시 추락 위험 | 높은 위치에서 작업 시 추락 위험이 있음 | ① 안전대 착용 및 고리 연결 ② 방호망 설치 |
    """
    if client is None:
        ...
        if client is None:
            ...
            if analysis_results:
                ...
                if len(analysis_results) > 1:
                    ...
                    combined_csv = combined_df.to_csv(index=False, encoding='utf-8-sig')
                    st.download_button(
                        label="📦 전체 결과 통합 CSV 다운로드",
                        data=combined_csv,
                        file_name=f"위험성평가_전체결과_{pd.Timestamp.now().strftime('%Y%m%d_%H%M%S')}.csv",
                        mime="text/csv",
                        use_container_width=True
                    )
else:
