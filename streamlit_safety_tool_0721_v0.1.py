...
def parse_analysis_sections(analysis_text: str) -> dict:
    """
    GPT 분석 결과를 5개 섹션으로 구분하여 파싱하는 함수
    ...
    if st.button(f"📊 {analysis_mode} - 종합 위험성 평가서 생성", type="primary", use_container_width=True):
        if client is None:
            ...
            try:
                ...
                with tab4:
                    st.subheader("현장 사진별 관찰 사항")
                    if sections.get("photo_observations"):
                        st.markdown(sections["photo_observations"])
                        if "photo_observations" in section_files:
                            st.download_button(
                                label="📥 사진 관찰사항 다운로드 (.md)",
                                data=section_files["photo_observations"].encode('utf-8-sig'),
                                file_name=f"사진관찰사항_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md",
                                mime="text/markdown",
                                key="photo_download"
                            )
                    else:
                ...
                if section_files:
                    ...
                    with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zip_file:
                        for file_name, content in section_files.items():
                            korean_names = {
                                "work_environment": "작업환경설명",
                                "risk_analysis": "위험요인분석",
                                "sgr_checklist": "SGR체크리스트", 
                                "recommendations": "추가권장사항",
                                "photo_observations": "사진관찰사항"
                            }
                            zip_file.writestr(
